...
export default function ChatbotPage() {
  ...
  const startMockInterview = async () => {
    ...
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/interview/start`, {
        method: 'POST',
      ...
      // Store context for the dedicated test page
      localStorage.setItem('interview_data', JSON.stringify({
      ...
      setMockError('AI quota is temporarily exhausted. Redirecting to test page with fallback questions.');
      
      // Fallback data if API fails
      localStorage.setItem('interview_data', JSON.stringify({
        questions: ["Explain your technical background.", "Describe a complex problem you solved.", "How do you handle technical debt?", "What are your strengths?", "Where do you see yourself in 5 years?"],
        company: currentCompany || 'Standard',
        role: currentRole,
        difficulty: difficulty
      }));
  ...
  return (
    <div className="flex h-screen t-bg-base t-text font-sans overflow-hidden relative" style={{ color: 'var(--text-primary)' }}>
      ...
      <aside className={`fixed lg:relative inset-y-0 left-0 w-72 t-bg-base border-r-2 border-black/10 dark:border-white/10 lg:mr-6 flex flex-col transition-all duration-300 z-50 backdrop-blur-xl shadow-md ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <div className="p-8 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-black tracking-tight">AI <span className="text-purple-500">INTERVIEW</span></h1>
            <p className="text-[10px] text-neutral-500 font-bold uppercase tracking-widest">Master Your Career</p>
          </div>
        </div>

        {/* NAVIGATION SECTION */}
        <div className="px-4 mb-6">
          <p className="px-4 text-[10px] text-neutral-600 font-black uppercase tracking-widest mb-3">Main Menu</p>
          <nav className="flex flex-col gap-1">
            <button 
              onClick={() => { startNewChat(); setIsMockMode(true); }}
              className={`sidebar-item ${isMockMode && !currentRole ? 'sidebar-item-active' : 'sidebar-item-hover t-text-sec hover:t-text'}`}
            >
              <Award size={20} />
            ...
            <button 
              onClick={() => { setShowSettings(!showSettings); setShowHistory(false); setShowProfile(false); }}
          ...
          <div className="px-4 mb-6">
            <div className="flex items-center justify-between px-4 mb-3">
              <p className="text-[10px] text-neutral-600 font-black uppercase tracking-widest">Active Filters</p>
              {(currentRole || currentCompany) && (
                <button onClick={() => { setCurrentRole(null); setCurrentCompany(null); }} className="text-[9px] text-purple-500 font-bold hover:underline">Clear</button>
              )}
            </div>
            ...
            <div className="flex flex-col gap-2">
              {currentCompany && (
                <div className="px-4 py-3 rounded-2xl bg-purple-600/10 border border-purple-500/20 flex items-center gap-3">
                  <div className={`w-6 h-6 rounded-lg bg-gradient-to-br ${companies.find(c => c.id === currentCompany)?.color} flex items-center justify-center text-[10px] font-bold text-white shadow-sm`}>
        ...
        <div className="p-4 mt-auto">
          ...
          >
            ...
            <div className="flex-1 text-left min-w-0">
              ...
              <p className="text-[10px] text-neutral-500 truncate uppercase tracking-tight">Personal Account</p>
            </div>
          </button>
      ...
      <aside className={`fixed lg:relative inset-y-0 left-72 lg:left-0 transition-all duration-300 border-r t-bg-panel flex flex-col z-40 ${showHistory ? 'w-72 lg:w-80 opacity-100' : 'w-0 opacity-0 overflow-hidden'} ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`} style={{ borderColor: 'var(--border)' }}>
        <div className="p-6 pt-24 lg:pt-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-xl font-bold mb-1 flex items-center gap-2">History</h2>
              <p className="text-[10px] text-neutral-600 font-bold uppercase tracking-widest">Control Center</p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between gap-2 mb-4">
            <button 
              ...
              className="flex-1 px-3 py-2 rounded-lg bg-white/5 border border-white/5 text-[10px] font-bold uppercase tracking-widest hover:bg-white/10 transition-all flex items-center justify-center gap-2"
            >
            ...
            {selectedChats.length > 0 && (
              <button 
                ...
                className="flex-1 px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/20 text-[10px] font-bold uppercase tracking-widest text-red-500 hover:bg-red-500/20 transition-all flex items-center justify-center gap-2"
              >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between">
                      <p className="text-[9px] text-neutral-600 font-bold uppercase">{new Date(chat.createdAt).toLocaleDateString()}</p>
                      <span className={`text-[8px] font-black px-1.5 py-0.5 rounded uppercase bg-neutral-800 text-neutral-400`}>
      ...
      {/* 3. MAIN DASHBOARD (Premium Floating Panel) */}
      <main className="flex-1 flex flex-col relative overflow-hidden lg:mr-6 lg:my-6 lg:rounded-[48px] shadow-2xl border t-border-strong backdrop-blur-sm" style={{ backgroundColor: 'var(--bg-base)' }}>
          ...
          <div className="flex-1 flex flex-col p-8 lg:p-12 overflow-y-auto items-center relative animate-in fade-in duration-500">
            <div className="max-w-2xl w-full">
              ...
              <div className="text-center mb-10">
                ...
                <p className="text-neutral-500 uppercase tracking-widest text-[10px] font-black">{user?.email}</p>
              </div>
              ...
              <div className="p-8 rounded-[40px] t-bg-card border light-card-shadow space-y-6 relative overflow-hidden" style={{ borderColor: 'var(--border)' }}>
                {!isEditing && (
                  <button 
                    ...
                    className="absolute top-6 right-6 flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/5 text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all text-neutral-400 hover:text-white"
                  >
                ...
                <div className="space-y-4 pt-4">
                  <div>
                    <label className="text-[10px] text-neutral-600 uppercase font-black tracking-widest mb-2 block">Username / Name</label>
                    {isEditing ? (
                      <input 
                        type="text" 
                  ...
                  <div>
                    <label className="text-[10px] text-neutral-600 uppercase font-black tracking-widest mb-2 block">Account Email</label>
                    {isEditing ? (
                      <input 
                        type="email" 
                  ...
                  {isEditing && (
                    <div>
                      <label className="text-[10px] text-neutral-600 uppercase font-black tracking-widest mb-2 block">Profile Image URL</label>
                      <input 
                        type="text" 
                  ...
                  <div>
                    <label className="text-[10px] text-neutral-600 uppercase font-black tracking-widest mb-2 block">Bio / About</label>
                    {isEditing ? (
                      <textarea 
                        value={editData.bio || ''} 
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-[10px] t-text-muted mt-0.5">Easy on the eyes</p>
                    </div>
                  ...
                  >
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-[10px] t-text-muted mt-0.5">Clean & bright</p>
                    </div>
        ...
        ) : isMockMode && wizardStep <= 5 && !activeThreadId ? (
              ...
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] t-text-muted font-black uppercase tracking-widest">Step {wizardStep > 4 ? 4 : wizardStep} of 4</p>
                  </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {companies.filter(c => c.category === selectedCompanyType || (selectedCompanyType === 'Services' && c.category === 'Services')).map(comp => (
                        ...
                        >
                          ...
                          <div>
                            ...
                            <p className="text-[10px] t-text-muted font-black uppercase tracking-widest">{comp.difficulty} Patterns</p>
                          </div>
                        </button>
                      ))}
                      <button 
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-[10px] t-text-muted font-black uppercase tracking-widest">Standard Patterns</p>
                        </div>
                      </button>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {roles.map(role => (
                        ...
                        >
                          ...
                          <p className="text-[10px] t-text-muted font-black uppercase tracking-widest">{role.category} Specialist</p>
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="space-y-12 animate-in zoom-in duration-500">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                      <div className="space-y-8">
                        <div>
                          <p className="text-[10px] text-neutral-600 font-black uppercase tracking-widest mb-4 px-1">Interview Setup Completed</p>
                          <div className="p-8 rounded-[48px] t-bg-card border t-border-strong light-card-shadow space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <div className="p-4 rounded-3xl t-bg-input">
                                <p className="text-[9px] t-text-muted font-black uppercase mb-1">Role</p>
                                <p className="font-bold">{roles.find(r => r.id === currentRole)?.title}</p>
                              </div>
                              <div className="p-4 rounded-3xl t-bg-input">
                                <p className="text-[9px] t-text-muted font-black uppercase mb-1">Experience</p>
                        ...
                        <div className="space-y-4">
                          <p className="text-[10px] text-neutral-600 font-black uppercase tracking-widest px-1">Focus Areas</p>
                        ...
                        >
                          {isLoading ? (
                            ...
                            <>
                              ...
                              <span className="text-[10px] opacity-60 uppercase tracking-widest">or click here</span>
                            </>
                          )}
                        </button>
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-6 lg:p-12 space-y-8 scroll-smooth custom-scrollbar">
              {mockState === 'finished' && mockReport ? (
                ...
                <div className="max-w-4xl mx-auto space-y-12 animate-in zoom-in duration-700 pb-20">
                  <div className="text-center space-y-4">
                    <div className="inline-flex items-center gap-2 px-6 py-2 rounded-full bg-green-500/10 border border-green-500/20 text-green-500 text-[10px] font-black uppercase tracking-widest">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="md:col-span-2 p-10 rounded-[48px] t-bg-card border t-border-strong shadow-2xl space-y-10">
                      <div className="grid grid-cols-2 gap-8">
                        <div>
                          <p className="text-[10px] t-text-muted font-black uppercase tracking-widest mb-2">Total Score</p>
                          <p className="text-6xl font-black text-purple-500">{mockReport.match(/Total Score: ([\d/]+)/)?.[1] || '39/50'}</p>
                        </div>
                        <div>
                          <p className="text-[10px] t-text-muted font-black uppercase tracking-widest mb-2">Overall Percentage</p>
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="p-8 rounded-[40px] t-bg-card border t-border-strong space-y-4">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="text-[10px] font-black uppercase text-purple-500">Ready for Interview</p>
                      ...
                      <React.Fragment key={i}>
                        <div className="flex justify-start animate-in fade-in slide-in-from-bottom-2 duration-300">
                          <div className="max-w-[80%] p-6 rounded-[24px] bg-green-500/5 border border-green-500/20 mr-12" style={{ color: 'var(--text-primary)' }}>
                            <div className="flex items-center gap-2 mb-4 text-green-500 font-black text-[10px] uppercase tracking-widest bg-green-500/10 w-fit px-3 py-1 rounded-full"><Check size={12} /> Evaluation System</div>
                            <p className="leading-relaxed whitespace-pre-wrap">{evalPart}</p>
                            <p className="text-[9px] mt-4 opacity-40 font-bold uppercase tracking-widest">AI Interviewer • Feedback</p>
                          </div>
                        </div>
                        <div className="flex justify-start animate-in fade-in slide-in-from-bottom-2 duration-500 mt-4">
                          <div className="max-w-[80%] p-6 rounded-[24px] t-bg-card border t-border mr-12" style={{ color: 'var(--text-primary)' }}>
                            ...
                            <p className="text-[9px] mt-4 opacity-40 font-bold uppercase tracking-widest">AI Interviewer • Next Challenge</p>
                          </div>
                        </div>
                      </React.Fragment>
                    );
                  ...
                  return (
                    <div key={i} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                      <div className={`max-w-[80%] p-6 rounded-[24px] ${msg.sender === 'user' ? 'bg-purple-600 text-white ml-12' : isEval ? 'mr-12 bg-green-500/5 border-green-500/20' : 'mr-12 t-bg-card'}`} style={msg.sender !== 'user' ? { border: isEval ? '1px solid rgba(34, 197, 94, 0.2)' : '1px solid var(--border)', color: 'var(--text-primary)' } : {}}>
                        {isEval && <div className="flex items-center gap-2 mb-4 text-green-500 font-black text-[10px] uppercase tracking-widest bg-green-500/10 w-fit px-3 py-1 rounded-full"><Check size={12} /> Evaluation System</div>}
                        <p className="leading-relaxed whitespace-pre-wrap">{content}</p>
                        <p className="text-[9px] mt-4 opacity-40 font-bold uppercase tracking-widest">{msg.sender === 'user' ? 'You' : 'AI Interviewer'}</p>
                      </div>
                    </div>
                  );
                })
              ...
              {isOfflineMode && mockState !== 'finished' && (
                <div className="mx-auto max-w-lg mb-4 p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-amber-500 text-[10px] font-black uppercase tracking-widest text-center">
