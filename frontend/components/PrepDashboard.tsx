...
export default function PrepDashboard() {
  ...
  const generateQuestionsForTopic = async (topicTitle: string, topicContent: string) => {
    ...
    while (attempts < maxAttempts && !success) {
      ...
      try {
        ...
        if (result && Array.isArray(result.questions)) {
          const seenTexts = (await getAuthValue('practice_questions_seen_texts')) || [];
          
          const validSet = result.questions.filter((q: any) => {
            if (!q || typeof q !== 'object') return false;
            const qText = (q.question || '').trim();
            const aText = (q.answer || '').trim();
            const diff = (q.difficulty || '').trim();

            if (!qText || !aText || !diff) return false;
            ...
            const isDuplicate = seenTexts.some((text: string) => text.toLowerCase() === qText.toLowerCase());
            if (isDuplicate) {
              return false;
            }
          ...
          if (validSet.length >= 5) {
            const formattedSet = validSet.map((q: any, i: number) => ({
              id: i + 1,
              question: q.question,
              answer: q.answer,
            ...
            const newTexts = formattedSet.map((q: any) => q.question);
            await setAuthValue('practice_questions_seen_texts', [...seenTexts, ...newTexts]);

            const cachedQuestionsKey = `practice_questions_set_${topicTitle}`;
            await setAuthValue(cachedQuestionsKey, formattedSet);

            setActiveQuestions(formattedSet);
            setCurrentQuestionIndex(0);
            setShowAnswer(false);
            showToast('Generated fresh practice questions!', 'success');
            success = true;
            break;
          } else {
            console.warn(`Attempt ${attempts}: Generated set was invalid or had too few questions (${validSet.length} valid). Regenerating...`);
          }
        }
      } catch (err: any) {
    ...
    if (!success) {
      const fallbackQuestions = [
        { question: `Can you explain the main architectural concepts of ${topicTitle}?`, answer: `The core concepts of ${topicTitle} revolve around building highly decoupled, modular components that communicate via standard interfaces. This ensures separation of concerns, ease of scalability, and straightforward unit testing across all components of the system.`, explanation: `Focuses on modularity and architecture.`, difficulty: 'Medium', topic: topicTitle },
        { question: `What are the primary performance considerations when working with ${topicTitle}?`, answer: `Performance optimization in ${topicTitle} primarily requires minimizing network round-trips, utilizing efficient local and remote caching strategies (like Redis or browser cache), optimizing query execution paths, and reducing memory footprint through lazy loading of modules and assets.`, explanation: `Focuses on performance bottlenecks.`, difficulty: 'Hard', topic: topicTitle },
        { question: `How do you handle error states and exceptional conditions in ${topicTitle}?`, answer: `Error handling is managed through centralized error boundaries or exception filters, ensuring that failures are caught, logged with appropriate context, and converted to user-friendly messages while keeping the system stable and preventing resource leaks.`, explanation: `Focuses on resilience.`, difficulty: 'Medium', topic: topicTitle },
        { question: `What is the role of caching in optimizing ${topicTitle} applications?`, answer: `Caching stores frequently accessed data in faster, temporary storage media. It helps reduce latency, limits load on backend relational databases, and speeds up page load times for end users, contributing to a highly responsive and performant user interface.`, explanation: `Focuses on optimization.`, difficulty: 'Easy', topic: topicTitle },
        { question: `How do you ensure data security and integrity when implementing ${topicTitle}?`, answer: `Security is ensured by sanitizing all incoming inputs, using prepared statements to prevent injection, enforcing strict Role-Based Access Control (RBAC), encrypting sensitive data in transit and at rest, and validating requests via secure session tokens.`, explanation: `Focuses on application security.`, difficulty: 'Hard', topic: topicTitle }
      ];

      const cachedQuestionsKey = `practice_questions_set_${topicTitle}`;
      await setAuthValue(cachedQuestionsKey, fallbackQuestions);

      setActiveQuestions(fallbackQuestions);
      setCurrentQuestionIndex(0);
      setShowAnswer(false);
      showToast('Loaded valid fallback practice questions.', 'success');
    }
  ...
  const handleOpenMaterial = async (mat: any) => {
    ...
    const cachedQuestionsKey = `practice_questions_set_${mat.title}`;
    const cached = await getAuthValue(cachedQuestionsKey);
    
    if (cached && Array.isArray(cached) && cached.length >= 5) {
      setActiveQuestions(cached);
    } else {
      if (isOffline) {
        setActiveQuestions([]);
      } else {
        setActiveQuestions([]);
        generateQuestionsForTopic(mat.title, mat.content);
      }
    }
  ...
  const handleGenerateQuestions = async () => {
    ...
    const isOffline = typeof navigator !== 'undefined' && !navigator.onLine;
    if (isOffline) {
      showToast('Cannot generate new questions while offline.', 'error');
      return;
    }
    generateQuestionsForTopic(selectedMaterial.title, selectedMaterial.content);
  };
            ...
            <div className="p-4 sm:p-8 flex-1">
              ...
              <div className="prose prose-invert prose-p:text-neutral-300 prose-headings:text-white max-w-none">
                ...
                <div className="mt-12 border-t border-white/10 pt-8">
                  ...
                  {generatingQuestions ? (
                  ...
                  ) : activeQuestions && activeQuestions.length > 0 && activeQuestions[currentQuestionIndex] ? (
                    <div className="p-6 sm:p-8 rounded-3xl bg-neutral-900 border border-white/10 shadow-xl transition-all duration-300 relative overflow-hidden">
                      ...
                      <p className="text-lg font-bold text-white mb-6 relative z-10">{activeQuestions[currentQuestionIndex].question}</p>
                      ...
                      <div className={`overflow-hidden transition-all duration-500 ease-in-out ${showAnswer ? 'max-h-[1000px] opacity-100 mb-6' : 'max-h-0 opacity-0'}`}>
                        <div className="p-5 rounded-2xl bg-black/40 border-l-2 border-purple-500">
                          <p className="text-sm text-neutral-300 leading-relaxed font-medium mb-3">{activeQuestions[currentQuestionIndex].answer}</p>
                    ...
                    <div className="p-8 rounded-3xl bg-neutral-900 border border-white/10 flex flex-col items-center justify-center text-center">
                      <BrainCircuit size={40} className="text-purple-400 mb-4 opacity-50" />
                      <p className="text-sm font-bold text-neutral-400">Connect to internet once to generate practice questions.</p>
                    </div>
                  )}
                </div>
              </div>
